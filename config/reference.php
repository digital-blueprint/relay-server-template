 ...
 *     },
 * }
 * @psalm-type DbpRelayMakerConfig = array<mixed>
 * @psalm-type DbpRelayCoreConnectorOidcConfig = array{
 *     server_url?: scalar|null, // The base URL for the OIDC server (in case of Keycloak for the specific realm)
 *     required_audience?: scalar|null, // If set only tokens which contain this audience are accepted (optional)
 *     local_validation_leeway?: int, // How much the system time of the API server and the Keycloak server can be out of sync (in seconds). Used for local token validation. // Default: 120
 *     remote_validation?: bool, // If remote validation should be used. If set to false the token signature will be only checked locally and not send to the keycloak server // Default: false
 *     remote_validation_id?: scalar|null, // The ID of the client (client credentials flow) used for remote token validation (optional)
 *     remote_validation_secret?: scalar|null, // The client secret for the client referenced by client_id (optional)
 *     frontend_client_id?: scalar|null, // The client ID for the OIDC client (authorization code flow) used for API docs and other frontends provided by the API itself
 *     user_identifier_claims?: list<scalar|null>,
 *     set_symfony_roles_from_scopes?: bool, // Convert the token scopes to Symfony roles and set them on the user. By default, scopes will be converted to upper-case and prefixed with 'ROLE_SCOPE_', so 'some-scope' will result in the 'ROLE_SCOPE_SOME_SCOPE' Symfony role being set. // Default: true
 *     frontend_keycloak_server?: scalar|null, // Deprecated: No longer needed // The Keycloak server base URL
 *     frontend_keycloak_realm?: scalar|null, // Deprecated: No longer needed // The keycloak realm
 *     frontend_keycloak_client_id?: scalar|null, // Deprecated: Use "frontend_client_id" instead // The ID for the keycloak client (authorization code flow) used for API docs or similar
 ...
 *     build_info?: scalar|null, // Some string identifying the current build (commit hash)
 *     build_info_url?: scalar|null, // Some URL identifying the current build (URL to the commit on some git web interface)
 *     logo_path?: scalar|null, // Path to the logo (256x256) of the API frontend
 *     docs_title?: scalar|null, // The title text of the API docs page // Default: "Relay API Gateway"
 *     docs_description?: scalar|null, // The description text of the API docs page (supports markdown) // Default: "*part of the [digital blueprint](https://www.digital-blueprint.org) project*"
 *     queue_dsn?: scalar|null, // See https://symfony.com/doc/5.3/messenger.html#redis-transport // Default: ""
 *     lock_dsn?: scalar|null, // https://symfony.com/doc/5.3/components/lock.html // Default: ""
 * }
 ...
 *     dbp_relay_maker?: DbpRelayMakerConfig,
