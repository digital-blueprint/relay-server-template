 ...
 *     },
 * }
 * @psalm-type DbpRelayCoreConnectorOidcConfig = array{
 *     server_url?: scalar|Param|null, // The base URL for the OIDC server (in case of Keycloak for the specific realm)
 *     required_audience?: scalar|Param|null, // If set only tokens which contain this audience are accepted (optional)
 *     local_validation_leeway?: int|Param, // How much the system time of the API server and the Keycloak server can be out of sync (in seconds). Used for local token validation. // Default: 120
 *     remote_validation?: bool|Param, // If remote validation should be used. If set to false the token signature will be only checked locally and not send to the keycloak server // Default: false
 *     remote_validation_id?: scalar|Param|null, // The ID of the client (client credentials flow) used for remote token validation (optional)
 *     remote_validation_secret?: scalar|Param|null, // The client secret for the client referenced by client_id (optional)
 *     frontend_client_id?: scalar|Param|null, // The client ID for the OIDC client (authorization code flow) used for API docs and other frontends provided by the API itself
 *     user_identifier_claims?: list<scalar|Param|null>,
 *     set_symfony_roles_from_scopes?: bool|Param, // Convert the token scopes to Symfony roles and set them on the user. By default, scopes will be converted to upper-case and prefixed with 'ROLE_SCOPE_', so 'some-scope' will result in the 'ROLE_SCOPE_SOME_SCOPE' Symfony role being set. // Default: true
 *     frontend_keycloak_server?: scalar|Param|null, // Deprecated: No longer needed // The Keycloak server base URL
 *     frontend_keycloak_realm?: scalar|Param|null, // Deprecated: No longer needed // The keycloak realm
 *     frontend_keycloak_client_id?: scalar|Param|null, // Deprecated: Use "frontend_client_id" instead // The ID for the keycloak client (authorization code flow) used for API docs or similar
 *     authorization_attributes?: list<array{ // Default: []
 *         name?: scalar|Param|null,
 *         scopes?: list<scalar|Param|null>,
 *         claim?: scalar|Param|null,
 *     }>,
 * }
 * @psalm-type DbpRelayCoreConfig = array{
 *     build_info?: scalar|Param|null, // Some string identifying the current build (commit hash)
 *     build_info_url?: scalar|Param|null, // Some URL identifying the current build (URL to the commit on some git web interface)
 *     logo_path?: scalar|Param|null, // Path to the logo (256x256) of the API frontend
 *     docs_title?: scalar|Param|null, // The title text of the API docs page // Default: "Relay API Gateway"
 *     docs_description?: scalar|Param|null, // The description text of the API docs page (supports markdown) // Default: "*part of the [digital blueprint](https://www.digital-blueprint.org) project*"
 *     queue_dsn?: scalar|Param|null, // See https://symfony.com/doc/5.3/messenger.html#redis-transport // Default: ""
 *     lock_dsn?: scalar|Param|null, // https://symfony.com/doc/5.3/components/lock.html // Default: ""
 * }
 ...
 *     dbp_relay_core_connector_oidc?: DbpRelayCoreConnectorOidcConfig,
 *     dbp_relay_core?: DbpRelayCoreConfig,
 *     "when@dev"?: array{
 *         imports?: ImportsConfig,
 *         parameters?: ParametersConfig,
 *         services?: ServicesConfig,
